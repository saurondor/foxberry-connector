	...
	private void usbReaderConnectButtonActionPerformed(ActionEvent e) {
		...
		if (ReaderContext.isUsbConnected()) {
			try {
				...
				JOptionPane.showMessageDialog(this,
						"No se pudo desconectar el lector.",
						"Desconexión al lector USB", JOptionPane.ERROR_MESSAGE);
			}
		...
		if (readerTypeComboBox.getSelectedIndex() == 0) {
			try {
				...
				ReaderContext.connectFoxberry(
						readerBoxAddressTextField.getText(), null, null, null);
				disableUsbFields();
				JOptionPane.showMessageDialog(this, "Conexión exitosa.",
						"Conexión a Foxberry", JOptionPane.INFORMATION_MESSAGE);
				...
				JOptionPane
						.showMessageDialog(
								...
								"Nombre de dispositivo desconocido. "
										+ e1.getMessage(), "Error conectando",
								JOptionPane.ERROR_MESSAGE);
				e1.printStackTrace();
			} catch (IOException e1) {
				JOptionPane.showMessageDialog(this,
						"Error de datos. " + e1.getMessage(), "Errorde red",
						JOptionPane.ERROR_MESSAGE);
				e1.printStackTrace();
			}
		} else {
			try {
				...
				ReaderContext.connectSpeedway(
						readerBoxAddressTextField.getText(), null, null);
				disableUsbFields();
				JOptionPane.showMessageDialog(this, "Conexión exitosa.",
						"Conexión a Speedway", JOptionPane.INFORMATION_MESSAGE);
				readerBoxConnectButton.setText("Desconectar");
			} catch (UnknownHostException e1) {
				JOptionPane
						.showMessageDialog(
