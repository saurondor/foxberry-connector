...
public class JImportTags extends JDialog {
	...
	private void selectImportFileButtonActionPerformed(ActionEvent e) {
		...
		if (response == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				DefaultComboBoxModel<String> comboModel = new DefaultComboBoxModel<String>(sheets.toArray(new String[sheets.size()]));
	...
	private void updateSheetColumns() {
		List<String> columns = excelImporter.getColumnNames(sheetComboBox.getSelectedIndex());
		DefaultComboBoxModel<String> comboBoxModel = new DefaultComboBoxModel<String>((String[]) columns.toArray(new String[columns.size()]));
		bibComboBox.setModel(comboBoxModel);
		comboBoxModel = new DefaultComboBoxModel<String>((String[]) columns.toArray(new String[columns.size()]));
	...
	private void importButtonActionPerformed(ActionEvent e) {
		try {
			excelImporter.importTags(sheetComboBox.getSelectedIndex(), bibComboBox.getSelectedIndex(), epcComboBox.getSelectedIndex());
		...
		// ======== dialogPane ========
		{
			...
			// ======== contentPanel ========
			{
				contentPanel.setLayout(new FormLayout(new ColumnSpec[] {
						...
						FormFactory.DEFAULT_COLSPEC,
						FormFactory.LABEL_COMPONENT_GAP_COLSPEC,
						new ColumnSpec(Sizes.dluX(140)) }, new RowSpec[] {
				...
				selectImportFileButton.setText(bundle
						.getString("JImportTags.selectImportFileButton.text"));
				selectImportFileButton.setFont(new Font("Tahoma", Font.PLAIN,
						14));
				...
				importButton.setText(bundle
						.getString("JImportTags.importButton.text"));
